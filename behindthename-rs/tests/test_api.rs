...
fn test_rate_limit() {
    ...
    match sesh.request(req_3) {
        Allowed(r) => match sesh.request(req_4) {
            Allowed(s) => assert!(false, "third and fourth requests succeeded: {:?}, {:?}", r.text().unwrap(), s.text().unwrap()),
            Limited(i, n) => println!{"third request: {:?}; fourth request: limiter {}, {:?}", r.text().unwrap(), i, n},
...
fn test_json_lookup() {
    ...
    match sesh.request_json(req) {
        ...
        Allowed(Okay(JsonResponseBody::NameList(e))) => assert!(false, "request parsed as name list: {:?}", e),
...
fn test_json_random() {
    ...
    match sesh.request_json(req) {
        ...
        Allowed(Okay(JsonResponseBody::NameDetails(e))) => assert!(false, "request parsed as name details: {:?}", e),
        Allowed(NotAvailable(_)) => assert!(false, "request service unavailable"),
        Limited(i, n) => assert!(false, "request failed: limiter {}, {:?}", i, n),
        Error(e) => assert!(false, "request failed: {:?}", e)
    };
...
fn test_json_service_unavailable() {
    ...
    match sesh.request_json(req_3) {
        Allowed(NotAvailable(e)) => assert_eq!((e.error_code, e.error), (2usize, "service not available".to_string())),
        Allowed(Okay(r)) => match sesh.request_json(req_4) {
            Allowed(NotAvailable(e)) => assert_eq!((e.error_code, e.error), (2usize, "service not available".to_string())),
            Allowed(Okay(s)) => assert!(false, "third and fourth requests succeeded: {:?}, {:?}", r, s),
            Limited(i, n) => println!{"third request: {:?}; fourth request: limiter {}, {:?}", r, i, n},
            Error(e) => assert!(false, "fourth request failed: {:?}", e)
        }
        Limited(i, n) => println!{"third request: limiter {}, {:?}", i, n},
        Error(e) => assert!(false, "third request failed: {:?}", e)
